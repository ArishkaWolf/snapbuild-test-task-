...
export function Product() {
  ...
  <section className="product" id="product"><h2>Одна платформа — весь маркетинг</h2><p className="product__intro">Сайты, изображения, видео, баннеры и презентации — из одной идеи, в вашем стиле</p>
    <div className="product__grid">{cards.map(([title, text, src]) => <article className="product-card" key={title}><div className="product-card__image"><img src={src} alt="" loading="lazy" decoding="async" /></div><h3>{title}</h3><p>{text}</p></article>)}</div>
  </section></>;
}
