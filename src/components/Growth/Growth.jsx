...
export function UseCases() {
  ...
  return <section className="growth cases" id="cases">
    ...
    <div className="cases__layout"><div className="cases__tabs" role="tablist" aria-label="Сценарии использования">{roles.map((role, index) => <button id={`case-tab-${index}`} type="button" role="tab" aria-selected={active === index} aria-controls="case-panel" tabIndex={active === index ? 0 : -1} className={active === index ? 'is-active' : ''} onClick={() => setActive(index)} key={role.name}><span>{role.number}</span>{role.name}<i>↗</i></button>)}</div>
      <article id="case-panel" className="cases__story" role="tabpanel" aria-labelledby={`case-tab-${active}`}><span className="cases__eyebrow">Сценарий / {item.name}</span><h3>{item.title}</h3><p>{item.text}</p><div>{item.tags.map(tag => <span key={tag}>{tag}</span>)}</div></article>
    </div>
  </section>;
}

const stats = [
  ['10×', 'быстрее запуск материалов', 'От первого брифа до готовой кампании — за часы вместо недель.', imagePaths.impact[0]],
  ['100%', 'соответствие дизайн-системе', 'Цвета, шрифты, сетки и компоненты применяются автоматически.', imagePaths.impact[1]],
  ['−70%', 'рутинных дизайн-задач', 'Команда меньше пересобирает макеты и больше работает над новыми идеями.', imagePaths.impact[2]],
  ['1', 'источник правды для бренда', 'Все форматы используют единые правила и обновляются вместе с системой.', imagePaths.impact[3]],
];
export function Impact() {
  return <section className="growth impact" aria-label="Результаты внедрения"><div className="impact__intro"><h2>Скорость растёт — бренд остаётся</h2><p>Снэпбилд превращает дизайн-систему в рабочую среду для всей компании. Команды самостоятельно выпускают больше материалов, а бренд-команда сохраняет контроль без дополнительных согласований и ручных проверок.</p></div><div className="impact__grid">{stats.map(([value, label, description, image]) => <article key={label}><div className="impact__photo"><img src={image} alt="" loading="lazy" decoding="async" /></div><strong>{value}</strong><h3>{label}</h3><p>{description}</p></article>)}</div></section>;
}
...
export function Stories() {
  ...
  return <section className="growth stories" id="stories">
    ...
    <div className="stories__carousel">
      ...
      <div className="stories__grid" aria-live="polite"><article className="is-accent" key={item.name}>
...
export function Pricing() {
  ...
  return <section className="growth pricing" id="pricing"><div className="pricing__head"><div><h2>Начните с команды — масштабируйте на компанию</h2><p className="pricing__subtitle">Варианты подключения для команды, нескольких брендов и закрытого корпоративного контура.</p></div><div className="pricing__toggle" role="group" aria-label="Период оплаты"><button className={!annual ? 'is-active' : ''} aria-pressed={!annual} onClick={() => setAnnual(false)} type="button">Месяц</button><button className={annual ? 'is-active' : ''} aria-pressed={annual} onClick={() => setAnnual(true)} type="button">Год <i>−20%</i></button></div></div><div className="pricing__grid">{tiers.map(tier => <article className={tier.featured ? 'is-featured' : ''} key={tier.name}>{tier.featured && <span className="pricing__badge">Рекомендуем</span>}<h3>{tier.name}</h3><p className="pricing__note">{tier.note}</p><div className="pricing__price"><strong>{annual ? tier.year : tier.month}</strong>{tier.month.match(/\d/) && <span>₽ / месяц</span>}</div><ul>{tier.features.map(feature => <li key={feature}>✓ <span>{feature}</span></li>)}</ul><a href="#contact">Обсудить подключение <span>↗</span></a></article>)}</div></section>;
}
